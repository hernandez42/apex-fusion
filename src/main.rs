...
//! 
//! Usage:
//!   apex-fusion compress <tool_type> [options]
//!   apex-fusion stats <input>
//!   apex-fusion benchmark
...
fn main() {
    ...
    match args[1].as_str() {
        "compress" => {
            if args.len() < 4 {
                eprintln!("Usage: apex-fusion compress <tool_type> <content> [--session <id>]");
                return;
            }
        ...
        "file" => {
            if args.len() < 3 {
                eprintln!("Usage: apex-fusion file <tool_type> <file_path>");
                return;
            }
            let tool_type = &args[2];
        ...
        "stats" => {
            if args.len() < 3 {
                eprintln!("Usage: apex-fusion stats <input>");
                return;
            }
            let input = &args[2];
...
fn run_benchmark() {
    ...
    for (name, content) in test_cases {
        let tool_type = if name.contains("Bash") { "Bash" }
            else if name.contains("Read") { "Read" }
            else if name.contains("JSON") { "Bash" } // JSON treated as general
            else { "Read" };

        let result = fuse_compress(content, tool_type, "benchmark", None, &config);
        let savings = format!("{:.1}%", result.savings_pct);
        
        println!("{}: {} → {} chars ({} saved)", 
            name, result.original_len, result.compressed_len, savings);
    ...
    println!("\n=== Overall: {} → {} chars ({:.1}% saved) ===", 
        total_original, total_compressed, overall);
}
